...
def calculate_demographic_data(print_data=True):
    ...
    higher_ed_types = ["Bachelors", "Masters", "Doctorate"]

    # with and without `Bachelors`, `Masters`, or `Doctorate`
    higher_education_rich = (
        round(
            (df[(df["education"].isin(higher_ed_types)) & (df["salary"] == ">50K")]["education"].count() /
            df[df["education"].isin(higher_ed_types)]["education"].count()) * 100,
            1)
    )
    ...
    rich_percentage = (
        round(
            (df[(df["hours-per-week"] == min_work_hours) & (df["salary"] == ">50K")]["salary"].count() /
            df[df["hours-per-week"] == min_work_hours]["salary"].count()) * 100,
            1)
    )
